...
- `summary_grant_funding` — grant/federal funding by fund source with total amounts, transaction counts, and year ranges. For grant-funding TOTALS and source lists ("how much grant funding", "from which sources"), use this table — and when the user asks "how much" overall, compute the grand total IN SQL so it appears in the results instead of being estimated later, e.g.: SELECT COALESCE(fund, 'TOTAL - ALL GRANT FUNDS') AS fund, ROUND(SUM(total_amount), 2) AS total_amount FROM summary_grant_funding GROUP BY ROLLUP(fund) ORDER BY total_amount DESC. For BREAKDOWNS within grant money (by agency, payee, or year — e.g. "which agencies received CARES money"), query `expenditures` filtered on the SPECIFIC fund names (fund = 'CARES Coronavirus Relief Fund (CRF)', fund LIKE 'CDBG%', etc. — the fund values in summary_grant_funding). Never approximate grant money with fund LIKE '%grant%' — that misses federal, CARES, CDBG, HOME, stimulus and other grant funds that don't contain the word "grant".
