...
- For registered agent queries: Query contractor_profiles directly (it already has total_spend and sos_registered_agent). Filter WHERE sos_registered_agent IS NOT NULL to exclude entities without agents. The `payee` column in contractor_profiles uses original names — some may be duplicates of normalized names (e.g., both "LG&E" and "LOUISVILLE GAS & ELECTRIC COMPANY" exist). When ranking by spend for agent lookups, GROUP BY sos_registered_agent or use the contractor_profiles table as-is and exclude NaN agents.
