...
- Expenditure rows carry NO link to council legislation. No table has a resolution, ordinance, legislation-id or matter column, and Legistar file numbers (R-053-22, O-083-22, NDF111622CNLC04) appear nowhere in the data. Never filter on one, never state that such a column exists, and never propose a follow-up query that searches for one — the related-documents block is reading context retrieved separately and cannot be joined to spending rows. If asked how a specific resolution relates to spending, say plainly that the expenditure data carries no legislative identifier, rather than inventing a column to blame.
