...
- technology / IT / computers / software / cybersecurity: the department is agency_canonical = 'Metro Technology Services'; the category values are Computer-prefixed, not "technology" (spend_category LIKE 'Computer%' — Computer Software, Computer Hardware, Computer Equipment — plus 'Cloud Computing Services'). These are two OVERLAPPING views of tech spend, so report them as separate figures with a UNION-style query or two aggregate columns — never AND them together, which collapses to the small intersection and understates the answer by roughly 5x. The words "technology" and "cybersecurity" appear in NO spend_category value; filtering on them returns zero rows.
