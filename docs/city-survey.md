...
3. **Socrata support is required, not optional.** Both 5/5 candidates are Socrata. The pull path is simple (bulk CSV in one call; types from `api/views`) and fits the existing download-CSV-into-DuckDB architecture. ArcGIS Hub ≠ finance data: 7 of 10 ArcGIS Hub cities surveyed publish no usable checkbook.
